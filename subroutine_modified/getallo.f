      ...
      do while (icd > 0)
        ...
        if (a /= "*") then
          ...
          case (1)                      !! icd = 1  SUBBASIN command
            ...
            do j = 1,57    !! changed (54)[talemayehu] Added by [svalencia]
...
!! check the number of days for aggregation to compute smi for season
!! change in tropics  default 5 days	 added by [talemayehu] 
	  open(1958,file = 'window size.txt',status = 'unknown')
	  eof = 0
		do
		read(1958,*,iostat = eof)
		if (eof < 0) exit
		read(1958,*,iostat = eof)w_size
		if (eof < 0) exit
		read(1958,*,iostat = eof)smi_tr
		if (eof < 0) exit
